...
2. Remove the file name following the sample name by typing:

`:%s/_Aligned.sortedByCoord.out.bam//g` (in command mode).
3. Remove the path leading up to the file name by typing:

`:%s/\/home\/rc_training10\/unix_workshop\/rnaseq\/results\/STAR\/bams\///g` (in command mode).
